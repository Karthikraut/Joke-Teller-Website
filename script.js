...
function audioJoke(joke){
    VoiceRSS.speech({
        key: '7d12e5115ca04d2ab3240d1688f95be1', // Remove the single quotes around the API key
        src: joke ,
        hl: 'en-us',
        v: 'Linda',
        r: 0,
...
async function jokeTell(){
    let joke = '';
    const apiUrl = 'https://sv443.net/jokeapi/v2/joke/Programming?blacklistFlags=nsfw,racist,sexist';
    try{
        const response = await fetch(apiUrl);
        const data = await response.json(); //converting API DATA INTO JSON Object.
        if(data.setup){
            joke = `${data.setup} .... ${data.delivery}`; //Here We use Backtick Because we have to COmbineString and Variable
        } else{
            joke = data.joke;
        }
        //Text -To Speech
...
button.addEventListener('click' , jokeTell);


//Button Enabling After the audio Element is ended
